...
function EcmSelect(select) {
    ...
    output.onchange = e => {
	const s = this.selectedOption.value, v = output.value
	console.log('Change select output', typeof s, typeof v, s === v, this.selectedOption, output.value)
	if (String(s) !== String(v)) { this.selectOption({value: v}) }
	console.log('Changed select output', this.selectedOption, output.value)
    }
    // el.style.position = 'relative'

    select.replaceWith(el);
    console.log("Have DOM", el, self)
    ECM.setDOM(el, self) 
...
Object.assign(EcmSelect.prototype, {
    onchange() { }
})
...
EcmSelect.prototype.selectOption = function (opt) {
    ...
    console.log("seelctOption", opt, this.options)
    this.options.forEach(o => {
	  const s = (o.value === opt.value)
       // console.log('opt', s, o)
	  o.selected = s
	  if (s) { this.selectedOption = o ;
               Object.assign(opt, o) }
    })
...
EcmSelect.prototype.selectOptionElement = function (el) {
	...
	value: el.getAttribute("data-ecm-value")
     ...
     console.log('inside select option element', opt, eopt, this, this.element)
...
EcmSelect.prototype.addOption = function (opt, prep)  {
    ...
    // console.log('added option', opt, el, this, self, prep)
    if (ex) { 
...
EcmSelect.prototype.template = `<div data-ecm-select-wrapper \
 class="uk-padding-remove ecm-select uk-select"
...
EcmSelect.prototype.templateStyle = `
 ...
 .ecm-select [uk-spinner] { position:absolute; right:5px ; top: 5px }
  ...
  .ecm-select input.uk-form-danger {
